...
count=Counter(df["bmi_group"])
ax.pie(count.values(),labels=count.keys(),autopct=lambda p:f'{p:.2f}%')
ax.set_title("Percentage of Transactions by bmi group")
plt.show();
...
fig,ax=plt.subplots(figsize=(5,5))
count=Counter(df[" HbA1c_group"])
ax.pie(count.values(),labels=count.keys(),autopct=lambda p:f'{p:.2f}%')
ax.set_title("Percentage of Transactions by HbA1c_group")
plt.show();
...
# 
# Blood Glucose (0.43) and HbA1c (0.41) are the most significant predictors of diabetes risk.
# 
# Moderate correlations:
# 
# Age (0.27),  BMI (0.22), and Heart Disease (0.17) are all linked to increased diabetes risk, but with weaker correlations.
# 
# Interrelationships:
# 
# Age & BMI (0.35): Older individuals tend to have higher BMI.
# 
# Hypertension & Heart Disease (0.12): Mild correlation, as they often occur together.
# Blood Glucose & HbA1c (0.18): Both measure blood sugar at different times.
# 
# 
...
# The four values represent: True Negatives (TN) = 16762 → The model correctly predicted non-diabetic cases.False Positives (FP) = 155 → The model incorrectly predicted diabetic when the patient was actually non-diabetic (Type I Error).False Negatives (FN) = 623 → The model incorrectly predicted non-diabetic when the patient was actually diabetic (Type II Error).True Positives (TP) = 1,073 → The model correctly predicted diabetic cases.Findings and Interpretation High True Negative Rate The model performs exceptionally well in detecting non-diabetic cases, with 16762 correct predictions. Low False Positive Rate (155 cases) Very few non-diabetic individuals were misclassified as diabetic. This is a good outcome because it reduces unnecessary anxiety and medical tests for healthy individuals. Moderate False Negative Rate (623 cases) The model misses some diabetic cases, which is concerning because failing to diagnose diabetes can have severe health consequences. Good True Positive Count (1,073 cases) The model identifies a reasonable number of actual diabetic cases correctly, but there is room for improvement. Conclusion and Next Steps The model performs well overall, with a high accuracy in predicting non-diabetic cases.However, the 623 false negatives indicate that some diabetic cases are being missed. Possible improvements: Adjusting the classification threshold to increase sensitivity. Using a different evaluation metric, such as F1-score or Recall, to balance false negatives.Exploring feature importance to see if additional factors can improve diabetic detection.
